...
describe("effectiveDownloadArtifactDir", () => {
  it("keeps a configured directory as typed", () => {
    expect(
      effectiveDownloadArtifactDir("/mnt/downloads", "/mnt/fast/transcode"),
    ).toBe("/mnt/downloads");
  });

  it("derives a sibling of the default transcode dir when both are blank", () => {
    expect(effectiveDownloadArtifactDir("", "")).toBe(
  ...
  it("follows a custom transcode dir", () => {
    ...
    expect(effectiveDownloadArtifactDir("", "/transcode")).toBe(
      "/silo-download-artifacts",
    );
  });
