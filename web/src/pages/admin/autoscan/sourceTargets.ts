...
export interface SourceTargets {
   ...
   * already-resolvable Prairie paths while polling — so the UI must say "unknown"
...
/**
 * Every Prairie-native path this source can produce: rewrite targets first (they
...
export function sourceTargets(
  ...
  if (paths.length === 0) {
    // A native-path source needs no configured root: it emits Prairie paths
