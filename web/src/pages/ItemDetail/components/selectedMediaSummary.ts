...
export function resolveSelectedMediaSummary(
  ...
  const quality = selectedVersion
    ? pickBestAttributes([selectedVersion])
    : null;
  const selectedVariant = selectedVersion
    ? playbackVariants?.find((variant) =>
        (variant.parts ?? []).some((part) =>
          (part.versions ?? []).some(
            (version) => version.file_id === selectedVersion.file_id,
          ),
        ),
      )
    : undefined;
  const isMultipart =
    (selectedVariant?.part_count ?? 0) > 1 ||
    (selectedVariant?.parts?.length ?? 0) > 1;
  const variantDuration = selectedVariant?.total_duration ?? 0;
  const durationSeconds =
    isMultipart && variantDuration > 0
      ? variantDuration
