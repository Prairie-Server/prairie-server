...
interface MediaUserActionBarProps extends Omit<
  ActionBarProps,
  UserActionProps
> {
  item: ItemDetail;
 ...
 */
export default function MediaUserActionBar({
  item,
  ...props
}: MediaUserActionBarProps) {
  const isFavorite = item.user_state?.is_favorite ?? false;
  const inWatchlist = item.user_state?.in_watchlist ?? false;
  const { mutate: toggleWatched, isPending: isUpdatingWatched } =
    useWatchedStateMutation(item);
