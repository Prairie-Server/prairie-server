...
function builderCollectionType(
  raw: Collection["collection_type"] | undefined,
): "manual" | "smart" {
...
export function toUserCollectionBuilderValue(
  ...
  return createCollectionBuilderValue({
    ...
    collection_type: builderCollectionType(
...
export function toCreateCollectionBody(
...
export function toUpdateCollectionBody(
  value: CollectionBuilderValue,
): UpdateCollectionRequest {
...
function UserCollectionSummary({
  ...
  const { watch: displayWatch, media: displayMedia } =
    queryDefinitionToDisplayFilters(draft.display_query_definition);
  ...
  return (
    <Card className="surface-panel gap-0 rounded-[1.5rem] border-0 shadow-none">
      <CardHeader>
        ...
        <CardDescription>
          Preview and sharing stay visible while you edit.
        </CardDescription>
      </CardHeader>
...
export function UserCollectionForm({
  ...
  const [draft, setDraft] = useState(() =>
    toUserCollectionBuilderValue(collection),
  );
  ...
  const builderLibraries = libraries.map((lib) => ({
    id: lib.id,
    name: lib.name,
    type: lib.type,
  }));
        ...
        <section className="space-y-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              Limit this collection to specific libraries. Leave empty to span
              every library you can see.
            </p>
          </div>
      ...
      {!readOnly ? (
        <section className="space-y-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              Upload a custom poster or paste an image URL. Leave empty to fall
