...
function buildDownloadedSubtitlesQuery(
  filters: AdminDownloadedSubtitlesFilters,
): string {
  const params = new URLSearchParams();
  if (filters.provider) params.set("provider", filters.provider);
  if (filters.language) params.set("language", filters.language);
  if (filters.userId != null) params.set("user_id", String(filters.userId));
  if (filters.mediaFileId != null)
    params.set("media_file_id", String(filters.mediaFileId));
...
export function useAdminDownloadedSubtitles(
  filters: AdminDownloadedSubtitlesFilters,
) {
  return useQuery({
    queryKey: adminKeys.downloadedSubtitles(filters),
    queryFn: () =>
      api<AdminDownloadedSubtitlesResponse>(
        buildDownloadedSubtitlesQuery(filters),
      ).then(
        (data) =>
          data ?? {
            subtitles: [],
...
export function useAdminUpdateDownloadedSubtitle() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      id: number;
      patch: AdminUpdateDownloadedSubtitleRequest;
    }) =>
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({
        queryKey: ["admin", "downloadedSubtitles"],
      });
    },
    onError: (err) => {
      toast.error(
        err instanceof Error ? err.message : "Failed to update subtitle",
      );
    },
  });
}
...
export function useSubtitleProviders() {
  return useQuery({
    ...
    queryFn: () =>
      api<{ providers: SubtitleProviderConfig[] }>("/admin/subtitle-providers"),
    staleTime: ADMIN_STALE_TIME,
  });
}
...
export function useTestSubtitleProvider() {
  return useMutation({
    mutationFn: ({
      ...
      provider: string;
      config: SubtitleProviderTestRequest;
    }) =>
      api<SubtitleProviderTestResponse>(
        `/admin/subtitle-providers/${provider}/test`,
        {
          method: "POST",
          body: JSON.stringify(config),
        },
