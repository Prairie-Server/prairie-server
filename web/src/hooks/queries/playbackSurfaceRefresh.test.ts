...
describe("invalidatePlaybackSurfaceQueries", () => {
  it("marks playback-derived queries as invalidated", async () => {
    ...
    queryClient.setQueryData(progressKeys.list("in_progress"), {
      progress: [],
    });
    ...
    expect(
      queryClient.getQueryState(progressKeys.list("in_progress"))
        ?.isInvalidated,
    ).toBe(true);
    ...
    expect(
      queryClient.getQueryState(sectionKeys.homeItems("continue"))
        ?.isInvalidated,
    ).toBe(true);
    expect(
      queryClient.getQueryState(sectionKeys.library(42))?.isInvalidated,
    ).toBe(true);
    expect(
