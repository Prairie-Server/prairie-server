...
export function useItemMarkers(
  ...
  return useQuery({
    ...
    queryFn: () =>
      api<FileMarkersResponse>(
        `/markers/items/${encodeURIComponent(itemId ?? "")}`,
      ),
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (itemId) {
        ...
        void queryClient.invalidateQueries({
          queryKey: ["items", "watchDetail", itemId],
        });
        void queryClient.invalidateQueries({
          queryKey: adminKeys.markerItemHistory(itemId),
        });
        void queryClient.invalidateQueries({
          queryKey: adminKeys.markerHistoryRoot(),
        });
      }
    },
...
export function useItemMarkerHistory(
  ...
  const historyKey = itemId
    ? adminKeys.markerItemHistory(itemId)
    : adminKeys.markerItemHistory("");
  return useQuery({
