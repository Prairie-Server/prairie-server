...
export async function invalidateRatingSurfaceQueries(
  ...
  await queryClient.invalidateQueries({
    predicate: (query) => {
      ...
      const isSimilarItems =
        startsWith(key, recKeys.all) && key[1] === "similar";
...
function startsWith(queryKey: readonly unknown[], prefix: readonly unknown[]) {
  return (
    ...
    prefix.every((part, index) => part === queryKey[index])
  );
}
