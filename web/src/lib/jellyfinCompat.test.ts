...
import {
  ...
  normalizeJellyfinCompatVersion,
} from "./jellyfinCompat";

type JellyfinWebInstallStatus = Parameters<
  typeof hasPinnedJellyfinWebInstalled
>[0];

function status(
  overrides: Partial<NonNullable<JellyfinWebInstallStatus>> = {},
) {
  return {
...
describe("jellyfinCompat helpers", () => {
  ...
  it("detects when the installed Web UI matches the pinned version", () => {
    ...
    expect(
      hasPinnedJellyfinWebInstalled(status({ installed_version: "v10.11.6" })),
    ).toBe(true);
  });

  it("does not treat missing or outdated installs as the pinned version", () => {
    expect(
      hasPinnedJellyfinWebInstalled(status({ installed_version: "" })),
    ).toBe(false);
    expect(
      hasPinnedJellyfinWebInstalled(status({ installed_version: "10.11.5" })),
    ).toBe(false);
