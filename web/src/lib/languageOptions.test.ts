...
describe("languageOptions", () => {
  it("uses the definition-specific generated option set", () => {
    const options = namedLanguageOptionsFor(
  ...
  it("uses each nullable definition's context-specific unset copy", () => {
    ...
    expect(
      languageOptionsFor(SETTING_KEYS.PLAYBACK_SUBTITLE_LANGUAGE)[0],
    ).toEqual({
      value: "",
      label: "None",
    });
    expect(
      languageOptionsFor(SETTING_KEYS.CATALOG_METADATA_LANGUAGE)[0],
    ).toEqual({
      value: "",
