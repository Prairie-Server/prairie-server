...
export const CARD_QUICK_ACTION_MODES = [
  "both",
  "favorites",
  "watched",
] as const;

export type EnabledCardQuickActionMode =
  (typeof CARD_QUICK_ACTION_MODES)[number];
...
export const CARD_QUICK_ACTION_OPTIONS: ReadonlyArray<{
  ...
  label: string;
}> = (SETTING_DEFINITIONS[SETTING_KEYS.UI_CARD_QUICK_ACTIONS].values ?? []).map(
  (member) => ({
