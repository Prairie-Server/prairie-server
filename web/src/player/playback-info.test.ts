...
describe("playback info helpers", () => {
  it("formats a direct-play session with source metadata and live runtime stats", () => {
    ...
    expect(rowValue(sections, "Video Info", "Video resolution")).toBe(
      "3840x2160",
    );
    ...
    expect(rowValue(sections, "Current Source File", "Bitrate")).toBe(
      "22.5 Mbps",
    );
    expect(rowValue(sections, "Current Source File", "Video codec")).toBe(
      "HEVC Main 10",
    );
    ...
    expect(rowValue(sections, "Current Source File", "Audio bitrate")).toBe(
      "640 kbps",
    );
    expect(rowValue(sections, "Current Source File", "Audio channels")).toBe(
      "6",
    );
  ...
  it("reads copy-versus-transcode from the plan's transformations, not codec strings", () => {
    ...
    expect(rowValue(sections, "Player", "Play method")).toBe(
      "Direct Streaming",
    );
  ...
  it("labels a re-encoded stream as transcoded on both axes", () => {
    const sections = buildPlaybackInfoSections({
      streamUrl:
        "https://app.example.com/api/v1/playback/transcode/session/master.m3u8",
      plan: fixturePlanV3({
  ...
  it("formats full and unspecified source color ranges", () => {
    ...
    const unknown = buildPlaybackInfoSections({
      ...
      plan: fixturePlanV3(),
      currentSourceVersion: makeVersion({
        video_tracks: [{ color_range: "unknown" }],
      }),
      runtimeStats: {},
    });

    expect(rowValue(full, "Current Source File", "Color range")).toBe(
      "Full (pc)",
    );
    expect(rowValue(unknown, "Current Source File", "Color range")).toBe(
      "Unknown",
    );
  });
  ...
  it("shows the requested source when playback auto-switches to a lower version", () => {
    ...
    expect(rowValue(sections, "Player", "Auto-switched from")).toBe(
      "2160p HEVC DV",
    );
    expect(rowValue(sections, "Current Source File", "Video codec")).toBe(
      "H.264 High",
...
describe("qualityOptionsFromPlanV3", () => {
  ...
  it("offers no auto entry when the plan publishes a single rung", () => {
    const options = qualityOptionsFromPlanV3(
      fixturePlanV3({
        available_qualities: [
          { label: "original", bitrate_kbps: 128, preserves_source: true },
        ],
      }),
    );
