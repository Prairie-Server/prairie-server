...
export default function FolderBrowser({
  ...
  const { data, isLoading, isFetching, error, refetch } =
    useFilesystemBrowse(currentPath);
  ...
  useEffect(() => {
    ...
    queryClient
      ...
      .then((nextFolder) => {
        ...
        setValidationError("");
        setCurrentPath((prev) =>
          prev === nextFolder.path ? prev : nextFolder.path,
        );
      })
      .catch(() => {
  ...
  async function handleBrowseSubmit() {
    ...
    try {
      ...
      setValidationError(
        err instanceof Error ? err.message : "Failed to browse folder",
      );
    }
  }
    ...
    >
      <DialogContent className="overflow-x-hidden sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Navigate the server filesystem and select one or more folders to add
            to this library.
          </DialogDescription>
        </DialogHeader>
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={() =>
                            !isExisting && toggleSelected(entry.path)
                          }
                          disabled={isExisting}
                          title={
                            isExisting
