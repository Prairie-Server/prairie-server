...
describe("ProviderTile", () => {
  it("names the tile after the provider so it can be found as a group", () => {
    ...
    expect(
      screen.getByRole("group", { name: "OpenSubtitles" }),
    ).toBeInTheDocument();
  ...
  it("labels each connection state and exposes it for styling", () => {
    const { rerender } = render(
      <ProviderTile name="SubDL" state="connected" />,
    );
    ...
    rerender(
      <ProviderTile
        name="SubDL"
        state="error"
        meta="401 — key rejected 2h ago"
      />,
    );
    expect(screen.getByText("Error")).toBeInTheDocument();
    expect(screen.getByText("401 — key rejected 2h ago")).toBeInTheDocument();
    expect(screen.getByRole("group", { name: "SubDL" })).toHaveAttribute(
      "data-state",
      "error",
    );
  });
  ...
  it("hides the panel until the tile is expanded", () => {
    ...
    expect(screen.getByRole("group", { name: "TMDB" })).toHaveAttribute(
      "data-expanded",
      "true",
    );
  });
