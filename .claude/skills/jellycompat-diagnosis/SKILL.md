...
description: Use when a Jellyfin-protocol client (JellyCon, Findroid, Infuse, Swiftfin) misbehaves against Prairie's jellycompat API — wrong or missing data, ignored sort/filter/pagination parameters, oversized responses, or slow browse queries. Covers comparing Prairie's responses against a real Jellyfin server field-by-field and reading the jellycompat debug log. Not for Prairie's own `/api/v1` clients.
---
...
identical request against Prairie and against a real Jellyfin, then diff the JSON.

Set `JELLYFIN_COMPARE_URL` and credentials in `.prairie-dev.env` — without a reference
...
```bash
scripts/prairie-dev sh 'grep -E "^(===|Status:|Remote:|Response \()" /opt/prairie/jellycompat-debug.log | tail -60'
```
...
Prairie's jellycompat listens on port 8096 alongside the main API.
