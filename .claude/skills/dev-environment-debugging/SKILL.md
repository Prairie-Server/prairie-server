...
description: Use when diagnosing a running Prairie server — playback, streaming, or transcode failures, unhealthy nodes, stuck sessions, wrong database state, errors after a deploy, or anything needing the deployment's logs, health endpoints, or SQL. Covers both a local docker compose install and a remote host over SSH. Not for building or testing code in this repo, and not for client-side bugs that reproduce without a server.
---

# Debugging a running Prairie deployment

Everything here goes through `scripts/prairie-dev`, which reads `.prairie-dev.env` and runs
commands against the deployment whether it is on this machine or behind SSH. Start with:

```bash
scripts/prairie-dev doctor
```
...
are really a misconfigured target. If `.prairie-dev.env` is missing, copy
`.prairie-dev.env.example` and fill it in; ask the user for hosts and credentials rather
than guessing, and never move those values into a committed file.

## Orienting

```bash
scripts/prairie-dev compose ps                  # what is running
scripts/prairie-dev api /api/v1/health          # API up
scripts/prairie-dev api /api/v1/ready           # PostgreSQL and optional S3 reachable
scripts/prairie-dev logs --tail 200             # recent server logs
scripts/prairie-dev logs -f                     # follow
```

`/ready` failing usually means the database, not the API — read the service logs before
concluding Postgres is down.

Prairie also records structured logs in the database itself, which survive container
...
Meilisearch is optional — Prairie runs without it — but it is recommended, and it is what
...
selected but the URL is empty, unreachable, or the provider fails to initialize, Prairie
...
```bash
scripts/prairie-dev psql "SELECT key, value FROM server_settings WHERE key LIKE 'catalog.search%' ORDER BY key;"
scripts/prairie-dev logs --tail 500 | grep -i 'catalog search'
```
...
(`scripts/prairie-dev sh 'ls -la …'`, under `PRAIRIE_MEDIA_ROOT`), then compare the file's
...
```bash
scripts/prairie-dev compose "exec -T prairie sh -lc 'ffmpeg -hwaccels; ls -la /dev/dri'"
```

Transcode output lands in a bind-mounted temp directory; a full disk there looks like a
transcode bug. If `PRAIRIE_TRANSCODE_SSH` is set, the node is a separate host with its own
...
```bash
scripts/prairie-dev compose ps
scripts/prairie-dev api /api/v1/ready
scripts/prairie-dev logs --tail 100
```
...
Prairie was formerly Silo, and Silo was formerly Continuum. Names like `continuum-postgres`, `/opt/continuum`,
