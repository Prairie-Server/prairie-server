...
**Tracking:** https://github.com/prairie-server/prairie-server/issues/376
...
`X-Prairie-Settings-Contract-Version` header checked on every authenticated request. It is withdrawn
...
- Clients filter definitions, scopes, enum members, and bounds against the server's advertised
  revision.
- Clients may send `X-Prairie-Settings-Contract-Revision` for telemetry about deployed revision
